...
@convert_labeler.message(attachment='photo')
...
@convert_labeler.raw_event(GroupEventType.MESSAGE_EVENT, MessageEvent, PayloadRule({'type': 'convert'}))
async def convert_photos(event: MessageEvent, user_api: API, file_storage: FileStorage, scheduler: Scheduler,
    ...
    photos = []
    message = (await bot_config.api.messages.
    ...
    files = await download_files(message, server, photos, scheduler, file_storage, localization, config, userdata)
    ...
    if result.status:
                                               ...
                                               user_api, bot_config.api, localization, event.peer_id)
