...
convert_labeler = bot_config.labelers.new()
convert_labeler.vbml_ignore_case = True


@convert_labeler.message(attachment='audio')
async def audio_handler(message: Message, localization, userdata):
    user = (await convert_labeler.api.users.get(user_ids=[message.from_id]))[0]
...
@convert_labeler.raw_event(GroupEventType.MESSAGE_EVENT, MessageEvent, PayloadRule({'type': 'audio_convert'}))
async def audio_convert_handler(event: MessageEvent, file_storage: FileStorage, scheduler: Scheduler,
    ...
    warning_tid = ''
    message = (await convert_labeler.api.messages
               .get_by_conversation_message_id(peer_id=event.peer_id, conversation_message_ids=payload.msg_id)
               ).items[0]
    user = (await convert_labeler.api.users.get(user_ids=[event.user_id]))[0]
    nickname = userdata.nickname or f'{user.first_name} {user.last_name}'
    audios = [('audio', i.audio) for i in message.attachments]
    files = await download_files(message, server, audios, scheduler, file_storage, localization, config, userdata)
