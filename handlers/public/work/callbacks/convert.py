...
async def work_convert(cbq: CallbackQuery, server: Server, callback_data: WorkCallback, scheduler: Scheduler,
    ...
    if result:
        if result.status:
            await cbq.message.reply_document(
                FSInputFile(result.content.result.path), caption=user_localization.TID_STARTWORK_DONE.format(
                    name=user_data.nickname or cbq.from_user.first_name
                ))
        else:
            await cbq.message.reply(user_localization[getattr(result, 'error_msg', 'TID_ERROR')].format(
                name=user_data.nickname or cbq.from_user.first_name
            ))
        shutil.rmtree(result.content.process_dir)
    else:
        await cbq.message.reply(user_localization.TID_ERROR.format(
            name=user_data.nickname or cbq.from_user.first_name
        ))
